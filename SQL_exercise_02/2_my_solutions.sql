...
SELECT Departments.Name FROM Employees JOIN Departments on Employees.Department == Departments.Code GROUP BY Departments.Code HAVING count(*) > 2;

-- 2.15 Very Important - Select the name and last name of employees working for departments with second lowest budget.
SELECT Name, LastName FROM Employees WHERE Department IN (
	SELECT Code FROM (SELECT Code, min(Budget) FROM Departments WHERE Budget > (SELECT min(Budget) FROM Departments))
);
...
DELETE FROM Employees WHERE Department IN (
	SELECT Departments.Code FROM Employees JOIN Departments on Employees.Department == Departments.Code WHERE Departments.Budget >= 60000
);
